...
func BuildUsageDocument(publicBaseURL string) UsageDocument {
	...
	return UsageDocument{
		...
		Version:     "1.0",
		Description: "Multi-tenant HTTP gateway with isolated route management, explicit route access modes, short-lived signed links, and reverse proxy forwarding.",
		Conventions: map[string]any{
			...
			"accessModes": []string{
				"public: forward directly.",
				"caller_token: require Access-Token and verify it with auth-service.",
				"signed_link: require gw_expires and gw_signature query parameters.",
			},
			"headerNaming": []string{
				"Access-Token",
				"Service-Name",
		...
		Endpoints: []EndpointDoc{
			...
			},
			{
				...
				Path:         "/api/routes",
				AuthRequired: true,
				Description:  "Create a route. If accessMode is caller_token or signed_link, authServiceName is required and must be managed by the bound service group.",
			...
			},
			{
				...
				Path:         "/api/routes/{id}",
				AuthRequired: true,
				Description:  "Update a route owned by the current user. Protected routes revalidate service-group permission.",
				RequestBody:  routeFields(),
			...
			{
				Method:       http.MethodPost,
				Path:         "/api/routes/{id}/signed-link",
				AuthRequired: true,
				Description:  "Create a short-lived signed URL for a signed_link route owned by the current user.",
				RequestBody: []FieldDoc{
					{Name: "method", Type: "string", Required: false, Description: "HTTP method to sign. Defaults to GET."},
					{Name: "path", Type: "string", Required: false, Description: "Path below /gw/{userSlug}. Defaults to route pathPattern."},
					{Name: "query", Type: "string", Required: false, Description: "Business query string. Do not include gw_expires or gw_signature."},
					{Name: "expiresInSeconds", Type: "number", Required: false, Description: "TTL in seconds. Defaults to 3600 and is capped at 86400."},
				},
				ResponseBody: []FieldDoc{
					{Name: "url", Type: "string", Required: true, Description: "Absolute signed gateway URL."},
					{Name: "method", Type: "string", Required: true, Description: "Signed HTTP method."},
					{Name: "expiresAtUnix", Type: "number", Required: true, Description: "Unix seconds expiry timestamp."},
				},
			},
		},
		Gateway: GatewayDoc{
			...
			Headers: []FieldDoc{
				{Name: "Access-Token", Type: "string", Required: false, Description: "Required for caller_token routes; verified by auth-service before forwarding."},
				{Name: "Service-Name", Type: "string", Required: false, Description: "For caller_token routes, forwarded to auth-service when present; otherwise route authServiceName is used."},
				{Name: "Origin", Type: "string", Required: false, Description: "Forwarded to auth-service for caller_token verification."},
			...
			Behaviors: []string{
				"public routes forward directly to upstream.",
				"caller_token routes require Access-Token and reject missing or failed tokens with 401 or 403.",
				"signed_link routes require gw_expires and gw_signature query parameters and strip them before forwarding upstream.",
				"Route matching is isolated by userSlug and never searches another user's routes.",
			},
			PrivateData: "The public usage document does not include user route configuration, authorization codes, tokens, or signing keys.",
		},
...
func routeFields() []FieldDoc {
	return []FieldDoc{
		...
		{Name: "accessMode", Type: "string", Required: false, Description: "public, caller_token, or signed_link. If omitted, legacy authRequired is used."},
		...
		{Name: "authRequired", Type: "boolean", Required: false, Description: "Legacy compatibility field. true maps to caller_token when accessMode is omitted."},
		{Name: "authServiceName", Type: "string", Required: false, Description: "Required when accessMode is caller_token or signed_link; used as Service-Name fallback for caller_token and ownership validation for protected routes."},
